	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<!-- <span class="l"> <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="admin_add('添加会员','member-add.html','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加会员</a> </span> -->
				...
				<c:forEach items="${userList}" var="user">
					<tr class="text-c">
						...
						<td class="td-manage">
							...
							<a title="编辑" href="javascript:;" onclick="admin_edit('会员编辑','member-add/${user.id}.html','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
							<a title="删除" href="javascript:;" onclick="admin_del(this,'1')" class="ml-5" style="text-decoration:none" data-user="${user.id}"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</td>
					</tr>
				</c:forEach>
				</tbody>
...
function admin_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: "post",
			url: "updateUserState.html",
			data: {
				'id': $(obj).attr("data-user"),
				'state': -1
			},
...
function admin_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		$.ajax({
			...
			url: "updateUserState.html",
			data: {
				'id': $(obj).next().next().attr("data-user"),
				'state': 0
			},
			success: function(result) {
				var jsonObj = JSON.parse(result);
